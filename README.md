...
口メッシュ（セカンダリ `7a73d3b5` など）を確実に拾うための手順。 Hunting モードで行う。

1. 3DMigoto の Hunting を ON（テンキー `0`）、キャラをプロフィール画面で正面に
2. `*` / `-` で VB をサイクルして **口なし** 状態にする → その状態で `F8` で FrameAnalysis を 1 枚取得
3. キャラ切り替え（別キャラ→対象キャラに戻す）を繰り返し、**口が表示された瞬間** に再度 `F8` で取得
4. 2 と 3 を交互に数回繰り返す（口なし / 口あり の両方が揃うまで）
5. `assets/Dump/<Char>/FrameAnalysis-*` に `*-vb0=*.buf` と `*-ib=*.buf` が溜まる。`Analyze Dump` で `c9846fd5 / 7a73d3b5` の両方が出れば成功

> ヒント: Hunting 中は `7` / `8` で IB サイクル、`9` でコピー。`ShaderUsage.txt` の `ps-t0` などは後で Body ゲートの確認に使える。
