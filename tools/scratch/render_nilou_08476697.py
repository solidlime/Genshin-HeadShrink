...
OUT_DIR = r"D:\Documents\Default Project\Nilou\render_test_08476697"
os.makedirs(OUT_DIR, exist_ok=True)

HEAD_SCALE = float(sys.argv[sys.argv.index('--') + 1]) if '--' in sys.argv and len(sys.argv) > sys.argv.index('--') + 1 else 0.65

HEAD_PREFIXES = ('Face', 'Brow', 'EyeStar', 'EffectMesh', 'EffectHair', 'Mask')
EXCLUDE_PREFIXES = ('Area_', 'Property_', 'DeathZone', 'AvatarObj_', 'Build_', 'Common_', 'NPC_', 'Ani_', 'SceneObj_', 'Stages_', 'MapObj_')

# Whitelist: 20 Nilou character meshes (Body/Face/Brow/EyeStar + Mask = Nilou's signature)
CHAR_MESH_WHITELIST = {
    'Body', 'Body_LOD1', 'Body_LOD2', 'Body_LOD3',
    'Face', 'Face_LOD2', 'Face_LOD3',
    'Face_Eye', 'Face_Eye_LOD2', 'Face_Eye_LOD3',
    'Brow', 'Brow_LOD2', 'Brow_LOD3',
    'EyeStar', 'EffectMesh',
    'Mask', 'Mask_LOD1', 'Mask_LOD2', 'Mask_LOD3',
}
...
def setup_scene_camera(cam_target, cam_size, mode='full'):
    ...
    mode='standing': full body portrait, camera at waist level looking up
    """
    ...
    if mode == 'head':
        ...
        # Full body portrait: camera at waist level (below head), looking up
        # This makes the character stand upright in the image
        cam_loc = Vector((cam_target.x + cam_dist * 0.25, cam_target.y - 0.3, cam_target.z + cam_dist * 1.6))
    else:  # full
...
for fname in sorted(os.listdir(OBJ_DIR)):
    ...
    if any(name.startswith(p) for p in EXCLUDE_PREFIXES):
        continue
    if name not in CHAR_MESH_WHITELIST:
        continue
    ...
    imported.append(obj)
print(f"[load] imported {len(imported)} meshes from {len(CHAR_MESH_WHITELIST)} whitelist")
...
# === Render 1: BASELINE (full body, standing) ===
# Aim just below head (Y=0.86m), use full character frame
cam_target = Vector((head_center.x, head_center.y - 0.15, head_center.z))
cam_size = Vector((0.9, 1.8, 0.9))
bpy.ops.object.select_all(action='DESELECT')
setup_scene_camera(cam_target, cam_size, mode='front')
