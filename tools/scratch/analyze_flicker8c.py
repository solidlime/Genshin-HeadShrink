...
for i,l in enumerate(lines):
    ...
    if pending_slot is not None:
        m = re.match(r'\d{6}\s+\d+:\s+resource=\S+ hash=([0-9a-f]+)', l)
        if m:
...
def prof(a,b,label):
    ...
    cnt = collections.Counter((ic,d[3]) for _,_,ic,d[3],_,_ in ds) if False else collections.Counter(ic for _,_,ic,_,_,_ in ds)
