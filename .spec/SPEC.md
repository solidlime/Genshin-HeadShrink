...
## T019: クロスキャラ汚染修正 — ゲートをキャラ固有ハッシュに限定 (2026-08-23, ユーザー提案)

**症状**: Sucrose mod 単独インストール (Mods 配下に Sucrose フォルダのみ) なのに Noelle の顔に影響が出る。

**根因 (dump FrameAnalysis-2026-08-23-091556 で実証)**:
- T015b で追加した IB ゲート \[TextureOverride*IB] hash=06e86a68 \=1\ の IB ハッシュがキャラ間共有 (F232 以降、同一ハッシュの別リソースが slot0/slot2 に同時 bind)。
- Noelle 表示中に SucroseIB セクションが発火し \=1 → 共有顔ハッシュの Sucrose 顔セクションの \if \ が通過 → Sucrose の Key (scale+offset) が Noelle の顔バッファに適用される。
- T015 当時は Key 正規化 (全キャラ同一変形) で汚染が不可視だったが、T018 の per-char offset 化で可視化された潜在バグ。

**設計 (ユーザー提案「キャラ固有ハッシュ→ゲート→発火時のみmod発動」を採用)**:
1. IB セクションから \ = 1\ を削除 (セクション自体は Head/Body IB 抽出のため handling=skip + drawindexed=auto + match_first_index 子セクションと共に残置)
2. \ のレイザーはキャラ固有リソースのみ: BodyGate (統合VB, キャラ毎に別ハッシュ) と Position (pre-skin 入力差替, キャラ固有)
...
## T020 差し戻し記録 (2026-08-23)

実機検証の結果、T020 (顔override廃止) では頭部統合メッシュのみ縮小しスタンドアロン顔パーツ (目/口/眉) が原寸のまま残り、近接表示で破綻すること判明。顔overrideは必須機能のため **commit 41bc339 (git revert e7b8644) で T019 状態へ復帰**。同時表示汚染は ini レベルで解決不可能な既知制約として受入する (緩和策: 同体型キャラ間で scale/face_offset を統一すれば汚染は無害化。単独表示は $is ゲートにより完璧に動作)。コミュニティ調査 (leotorrez ガイド「There is no solution」) とダンプ検証 (編成画面では VB・IB・vb1 まで完全共有) の詳細は nous 記憶 memory_20260823100134 / memory_20260823100455 参照。
