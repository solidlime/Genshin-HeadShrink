...
class UnitsSaveLoadTest(unittest.TestCase):
    def setUp(self):
        self._orig_path_fn = hs.face_offsets_path
        self.path = make_config_file(
            units={"def7af36": "BODY"},
            extra={"shrink_scale": 0.9})
    ...
    def test_save_load_roundtrip(self):
        ...
        ctx = make_ctx(props)
        # 追加 → 保存
        props.units_vb0 = "63f702ce"
        props.units_role = "EYES"
        run_op(hs.NHS_OT_UnitsAdd, ctx)
        props.units_vb0 = "def7af36"
        props.units_role = "BODY"
        run_op(hs.NHS_OT_UnitsAdd, ctx)
        op, result = run_op(hs.NHS_OT_UnitsSave, ctx)
        self.assertEqual(result, {'FINISHED'})
        # ファイルに units が正しく書かれている
        with open(self.path, encoding='utf-8') as f:
            data = json.load(f)
        units = data['TestChar']['__config__']['units']
        self.assertEqual(units, {"63f702ce": "EYES", "def7af36": "BODY"})
        # 他の config キーは保持されている
        self.assertEqual(data['TestChar']['__config__']['shrink_scale'], 0.9)
        # リストを空にして load → 復元される
        props.units_list.clear()
        op, result = run_op(hs.NHS_OT_UnitsLoad, ctx)
        self.assertEqual(result, {'FINISHED'})
        loaded = {i.vb0: i.role for i in props.units_list}
        self.assertEqual(loaded, {"63f702ce": "EYES", "def7af36": "BODY"})

    def test_save_preserves_face_offsets(self):
        # 元ファイルに face offset エントリがある状態で保存しても残る
        with open(self.path, encoding='utf-8') as f:
            data = json.load(f)
        data['TestChar']['Dump_def7af36.001'] = [0.0, 0.0, 0.5]
        ...
        props.units_vb0 = "def7af36"
        run_op(hs.NHS_OT_UnitsAdd, ctx)
        run_op(hs.NHS_OT_UnitsSave, ctx)
        with open(self.path, encoding='utf-8') as f:
            data = json.load(f)
        self.assertEqual(data['TestChar']['Dump_def7af36.001'],
                         [0.0, 0.0, 0.5])
        self.assertEqual(data['TestChar']['__config__']['units'],
                         {"def7af36": "BODY"})

    def test_load_empty_units(self):
        path = make_config_file()  # units なし
        try:
